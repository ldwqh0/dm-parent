...
public class SsoConfiguration extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/users/current", "/menuAuthorities/current", "/p/users/current",
						"/p/menuAuthorities/current")
				.authenticated().anyRequest().access("@authorityChecker.check(authentication,request)");
		// 设定匿名用户的用户实体
